...
{
  // brush block
  int wait= 50; //avoid instant unstable signal
  int wait1= 1000; //the interval of each ball

  int threshold= 1; //lower represent has hit
  int brush[]= {2,4,7,8,12,13};
  int count= 0; // current used ball
  int balls= 10; // the total ball number
  char *number_board[]= {"num1","num2","num3","num4","num5","num6","num7","num8","num9"};
  
  // String hit;
  // hit = "not hit";
  bool hit= 0;
  bool hit_condition= 0;
  
  // timing block
  double timing= 3; //the interval of time
  double time0 = micros()/1000000; // begining 
  double time1 = time0; // current time
  ...
  while (count<= balls)
  {
    ...
    if (a == true)
    {
        if (d== true)
        {
            delay(wait); //avoid the unstable signal
            if (d== true)
            {
                hit= number_board[0];
            }
    ...
    time1 = micros()/1000000; // renew the current time
  }
